...
def FUELLF1(x):
    result = (
        ...
        if extractor.apply_rule(
            "If the ship must have sufficient fuel for long-range missions.", x
        )
        == True
        else ABSTAIN
    )
...
def FUELLF2(x):
    result = (
        ClassLabels.FUEL
        if extractor.apply_rule(
            "If the fuel consumption must be optimized for efficiency.", x
        )
        == True
        else ABSTAIN
    )
...
def FUELLF3(x):
    result = (
        ClassLabels.FUEL
        if extractor.apply_rule("If the ship must have access to refueling points.", x)
        == True
        else ABSTAIN
    )
    if LOGGING_ENABLED and result != ABSTAIN:
...
def FUELLF4(x):
    result = (
        ClassLabels.FUEL
        if extractor.apply_rule(
            "If the fuel reserves must be managed for extended operations.", x
        )
        == True
        else ABSTAIN
    )
...
def FUELLF5(x):
    result = (
        ClassLabels.FUEL
        if extractor.apply_rule("If the ship must have emergency fuel supplies.", x)
        == True
        else ABSTAIN
    )
    if LOGGING_ENABLED and result != ABSTAIN:
...
def FUELLF6(x):
    result = (
        ClassLabels.FUEL
        if extractor.apply_rule(
            "If the fuel type must be compatible with mission requirements.", x
        )
        == True
        else ABSTAIN
    )
...
def FUELLF7(x):
    result = (
        ClassLabels.FUEL
        if extractor.apply_rule(
            "If the fuel consumption must be monitored for cost-effectiveness.", x
        )
        == True
        else ABSTAIN
    )
...
def FUELLF8(x):
    result = (
        ClassLabels.FUEL
        if extractor.apply_rule("If the ship must have fuel-efficient engines.", x)
        == True
        else ABSTAIN
    )
    if LOGGING_ENABLED and result != ABSTAIN:
...
def FUELLF9(x):
    result = (
        ClassLabels.FUEL
        if extractor.apply_rule("If the fuel storage must be secure against leaks.", x)
        == True
        else ABSTAIN
    )
    if LOGGING_ENABLED and result != ABSTAIN:
...
def FUELLF10(x):
    result = (
        ClassLabels.FUEL
        if extractor.apply_rule(
            "If the fuel must be of high quality to ensure engine performance.", x
        )
        == True
        else ABSTAIN
    )
