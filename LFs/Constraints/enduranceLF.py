...
def ENDURANCELF2(x):
    result = (
        ClassLabels.ENDURANCE
        if extractor.apply_rule("If the crew must endure long durations at sea.", x)
        == True
        else ABSTAIN
    )
    if LOGGING_ENABLED and result != ABSTAIN:
...
def ENDURANCELF3(x):
    result = (
        ClassLabels.ENDURANCE
        if extractor.apply_rule(
            "If the ship must have sufficient fuel reserves for prolonged missions.", x
        )
        == True
        else ABSTAIN
    )
...
def ENDURANCELF4(x):
    result = (
        ClassLabels.ENDURANCE
        if extractor.apply_rule(
            "If the ship must be capable of continuous surveillance for weeks.", x
        )
        == True
        else ABSTAIN
    )
...
def ENDURANCELF5(x):
    result = (
        ClassLabels.ENDURANCE
        if extractor.apply_rule("If the ship must endure harsh weather conditions.", x)
        == True
        else ABSTAIN
    )
    if LOGGING_ENABLED and result != ABSTAIN:
...
def ENDURANCELF6(x):
    result = (
        ClassLabels.ENDURANCE
        if extractor.apply_rule(
            "If the crew must maintain high alertness for extended periods.", x
        )
        == True
        else ABSTAIN
    )
...
def ENDURANCELF7(x):
    result = (
        ClassLabels.ENDURANCE
        if extractor.apply_rule(
            "If the ship must have reliable power generation for long missions.", x
        )
        == True
        else ABSTAIN
    )
...
def ENDURANCELF8(x):
    result = (
        ClassLabels.ENDURANCE
        if extractor.apply_rule(
            "If the ship must endure prolonged exposure to enemy threats.", x
        )
        == True
        else ABSTAIN
    )
...
def ENDURANCELF9(x):
    result = (
        ClassLabels.ENDURANCE
        if extractor.apply_rule(
            "If the ship must have sufficient provisions for extended voyages.", x
        )
        == True
        else ABSTAIN
    )
...
def ENDURANCELF10(x):
    result = (
        ClassLabels.ENDURANCE
        if extractor.apply_rule(
            "If the ship must endure continuous operational tempo.", x
        )
        == True
        else ABSTAIN
    )
