...
def SPEEDLF1(x):
    result = (
        ...
        if extractor.apply_rule(
            "If the ship must maintain high speed for rapid response.", x
        )
        == True
        else ABSTAIN
    )
...
def SPEEDLF2(x):
    result = (
        ClassLabels.SPEED
        if extractor.apply_rule(
            "If the speed must be optimized for fuel efficiency.", x
        )
        == True
        else ABSTAIN
    )
...
def SPEEDLF3(x):
    result = (
        ClassLabels.SPEED
        if extractor.apply_rule(
            "If the ship must have the capability for high-speed maneuvers.", x
        )
        == True
        else ABSTAIN
    )
...
def SPEEDLF4(x):
    result = (
        ClassLabels.SPEED
        if extractor.apply_rule("If the speed must be managed for mission success.", x)
        == True
        else ABSTAIN
    )
    if LOGGING_ENABLED and result != ABSTAIN:
...
def SPEEDLF5(x):
    result = (
        ClassLabels.SPEED
        if extractor.apply_rule(
            "If the ship must have emergency speed capabilities.", x
        )
        == True
        else ABSTAIN
    )
...
def SPEEDLF6(x):
    result = (
        ClassLabels.SPEED
        if extractor.apply_rule(
            "If the speed must be compatible with mission requirements.", x
        )
        == True
        else ABSTAIN
    )
...
def SPEEDLF7(x):
    result = (
        ClassLabels.SPEED
        if extractor.apply_rule(
            "If the ship must have reliable engines for high speed.", x
        )
        == True
        else ABSTAIN
    )
...
def SPEEDLF8(x):
    result = (
        ClassLabels.SPEED
        if extractor.apply_rule(
            "If the speed must be timely for emergency situations.", x
        )
        == True
        else ABSTAIN
    )
...
def SPEEDLF9(x):
    result = (
        ClassLabels.SPEED
        if extractor.apply_rule("If the ship must have speed-efficient hull design.", x)
        == True
        else ABSTAIN
    )
    if LOGGING_ENABLED and result != ABSTAIN:
...
def SPEEDLF10(x):
    result = (
        ClassLabels.SPEED
        if extractor.apply_rule(
            "If the speed must be managed for extended missions.", x
        )
        == True
        else ABSTAIN
    )
