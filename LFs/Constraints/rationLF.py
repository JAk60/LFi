...
def RATIONLF1(x):
    result = (
        ...
        if extractor.apply_rule(
            "If the ship must have sufficient food supplies for extended missions.", x
        )
        == True
        else ABSTAIN
    )
...
def RATIONLF2(x):
    result = (
        ClassLabels.RATION
        if extractor.apply_rule("If the rations must be balanced for crew health.", x)
        == True
        else ABSTAIN
    )
    if LOGGING_ENABLED and result != ABSTAIN:
...
def RATIONLF3(x):
    result = (
        ClassLabels.RATION
        if extractor.apply_rule(
            "If the food supplies must be managed for efficiency.", x
        )
        == True
        else ABSTAIN
    )
...
def RATIONLF4(x):
    result = (
        ClassLabels.RATION
        if extractor.apply_rule("If the ship must have emergency rations.", x) == True
        else ABSTAIN
    )
    if LOGGING_ENABLED and result != ABSTAIN:
...
def RATIONLF5(x):
    result = (
        ClassLabels.RATION
        if extractor.apply_rule(
            "If the rations must be compatible with crew dietary requirements.", x
        )
        == True
        else ABSTAIN
    )
...
def RATIONLF6(x):
    result = (
        ClassLabels.RATION
        if extractor.apply_rule(
            "If the food supplies must be secure against spoilage.", x
        )
        == True
        else ABSTAIN
    )
...
def RATIONLF7(x):
    result = (
        ClassLabels.RATION
        if extractor.apply_rule(
            "If the rations must be sufficient for high-intensity operations.", x
        )
        == True
        else ABSTAIN
    )
...
def RATIONLF8(x):
    result = (
        ClassLabels.RATION
        if extractor.apply_rule(
            "If the food supplies must be coordinated with mission timelines.", x
        )
        == True
        else ABSTAIN
    )
...
def RATIONLF9(x):
    result = (
        ClassLabels.RATION
        if extractor.apply_rule(
            "If the rations must be managed for cost-effectiveness.", x
        )
        == True
        else ABSTAIN
    )
