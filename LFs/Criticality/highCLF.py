...
def HighCritical_LF1(x):
    ...
    result = (
        ClassLabels.High
        if extractor.apply_rule("If direct lethal combat is imminent.", x)
        else ABSTAIN
    )
...
def HighCritical_LF2(x):
    ...
    result = (
        ClassLabels.High
        if extractor.apply_rule("If nuclear weapons are involved.", x)
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
    #     writer = csv.writer(f)
...
def HighCritical_LF3(x):
    ...
    result = (
        ...
        if extractor.apply_rule(
            "If mission requires immediate enemy neutralization.", x
        )
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
...
def HighCritical_LF4(x):
    ...
    result = (
        ClassLabels.High
        if extractor.apply_rule(
            "If personnel face immediate life-threatening risks.", x
        )
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
...
def HighCritical_LF5(x):
    ...
    result = (
        ClassLabels.High
        if extractor.apply_rule(
            "If strategic military assets are under direct threat.", x
        )
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
...
def HighCritical_LF6(x):
    ...
    result = (
        ClassLabels.High
        if extractor.apply_rule("If enemy combatants are actively engaging.", x)
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
    #     writer = csv.writer(f)
...
def HighCritical_LF7(x):
    ...
    result = (
        ClassLabels.High
        if extractor.apply_rule(
            "If mission involves capturing high-value military targets.", x
        )
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
...
def HighCritical_LF8(x):
    ...
    result = (
        ClassLabels.High
        if extractor.apply_rule(
            "If precision strikes against critical infrastructure required.", x
        )
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
...
def HighCritical_LF9(x):
    ...
    result = (
        ClassLabels.High
        if extractor.apply_rule("If potential for mass casualty scenario exists.", x)
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
    #     writer = csv.writer(f)
...
def HighCritical_LF10(x):
    ...
    result = (
        ClassLabels.High
        if extractor.apply_rule("If total military force projection is necessary.", x)
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
    #     writer = csv.writer(f)
