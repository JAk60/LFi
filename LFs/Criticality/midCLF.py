...
def MidCritical_LF1(x):
    ...
    result = (
        ClassLabels.Mid
        if extractor.apply_rule("If diplomatic resolution is still possible.", x)
        else ABSTAIN
    )
...
def MidCritical_LF2(x):
    ...
    result = (
        ...
        if extractor.apply_rule(
            "If potential conflict exists without direct engagement.", x
        )
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
...
def MidCritical_LF3(x):
    ...
    result = (
        ClassLabels.Mid
        if extractor.apply_rule(
            "If strategic positioning requires cautionary measures.", x
        )
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
...
def MidCritical_LF4(x):
    ...
    result = (
        ClassLabels.Mid
        if extractor.apply_rule("If military presence serves as deterrence.", x)
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
    #     writer = csv.writer(f)
...
def MidCritical_LF5(x):
    ...
    result = (
        ClassLabels.Mid
        if extractor.apply_rule("If intelligence gathering is primary objective.", x)
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
    #     writer = csv.writer(f)
...
def MidCritical_LF6(x):
    ...
    result = (
        ClassLabels.Mid
        if extractor.apply_rule(
            "If limited military intervention might be required.", x
        )
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
...
def MidCritical_LF7(x):
    ...
    result = (
        ClassLabels.Mid
        if extractor.apply_rule("If monitoring high-risk zones is necessary.", x)
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
    #     writer = csv.writer(f)
...
def MidCritical_LF8(x):
    ...
    result = (
        ClassLabels.Mid
        if extractor.apply_rule("If potential escalation risk is moderate.", x)
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
    #     writer = csv.writer(f)
...
def MidCritical_LF9(x):
    ...
    result = (
        ClassLabels.Mid
        if extractor.apply_rule("If protective security deployment is needed.", x)
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
    #     writer = csv.writer(f)
...
def MidCritical_LF10(x):
    ...
    result = (
        ClassLabels.Mid
        if extractor.apply_rule("If maritime territorial integrity is challenged.", x)
        else ABSTAIN
    )
    # with open(log_file, 'a', newline='') as f:
    #     writer = csv.writer(f)
