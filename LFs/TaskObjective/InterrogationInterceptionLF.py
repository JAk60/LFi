...
def InterrogationInterceptionLF1(x):
    ...
    result = (
        ClassLabels.InterrogationInterception
        if extractor.apply_rule(
            "If the operation involves direct interrogation of suspects.", x
        )
        == True
        else ABSTAIN
    )
...
def InterrogationInterceptionLF2(x):
    ...
    result = (
        ClassLabels.InterrogationInterception
        if extractor.apply_rule(
            "If the use of interception techniques is authorized for the mission.", x
        )
        == True
        else ABSTAIN
    )
...
def InterrogationInterceptionLF3(x):
    ...
    result = (
        ClassLabels.InterrogationInterception
        if extractor.apply_rule(
            "If the target is to be interrogated for critical information.", x
        )
        == True
        else ABSTAIN
    )
...
def InterrogationInterceptionLF4(x):
    ...
    result = (
        ClassLabels.InterrogationInterception
        if extractor.apply_rule(
            "If the situation demands immediate interception of communications.", x
        )
        == True
        else ABSTAIN
    )
...
def InterrogationInterceptionLF5(x):
    ...
    result = (
        ClassLabels.InterrogationInterception
        if extractor.apply_rule(
            "If the enemy is within interception range and poses a threat.", x
        )
        == True
        else ABSTAIN
    )
...
def InterrogationInterceptionLF6(x):
    ...
    result = (
        ClassLabels.InterrogationInterception
        if extractor.apply_rule(
            "If the tactical plan includes coordinated interrogation efforts.", x
        )
        == True
        else ABSTAIN
    )
...
def InterrogationInterceptionLF7(x):
    ...
    result = (
        ClassLabels.InterrogationInterception
        if extractor.apply_rule(
            "If the objective can only be achieved through interrogation.", x
        )
        == True
        else ABSTAIN
    )
...
def InterrogationInterceptionLF8(x):
    ...
    result = (
        ClassLabels.InterrogationInterception
        if extractor.apply_rule(
            "If the rules of engagement specify the use of interception methods.", x
        )
        == True
        else ABSTAIN
    )
...
def InterrogationInterceptionLF9(x):
    ...
    result = (
        ClassLabels.InterrogationInterception
        if extractor.apply_rule(
            "If the hostile forces are equipped with communication devices.", x
        )
        == True
        else ABSTAIN
    )
...
def InterrogationInterceptionLF10(x):
    ...
    result = (
        ClassLabels.InterrogationInterception
        if extractor.apply_rule(
            "If the mission requires continuous interception to gather intelligence.", x
        )
        == True
        else ABSTAIN
    )
...
InterrogationInterceptionLFS = [
    InterrogationInterceptionLF1,
    InterrogationInterceptionLF2,
    InterrogationInterceptionLF3,
    InterrogationInterceptionLF4,
    InterrogationInterceptionLF5,
    InterrogationInterceptionLF6,
    InterrogationInterceptionLF7,
    InterrogationInterceptionLF8,
    InterrogationInterceptionLF9,
    InterrogationInterceptionLF10,
]
