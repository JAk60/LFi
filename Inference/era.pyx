...
cdef class LabelingFunctionApplier:
    ...
    cpdef double voting_based_confidence(self, list votes):
        ...
        """
        cdef int total_len = len(votes)
        cdef list valid_votes = [v for v in votes if v is not None]
        cdef int count_1
        ...
        confidence = count_1 / total_len if valid_votes else 0.0

        return confidence

    cpdef double get_final_predictions(self, list data_points):
        """Wrapper to apply labeling functions and use the voting mechanism"""
        cdef list L
        cdef object S
        cdef double lf_confidences
        
        L, S = self.apply_lfs(data_points)
        lf_confidences = self.voting_based_confidence(L[0])
        return lf_confidences

cpdef dict load_lf_dictionary():
    return {
...
cpdef dict process_predictions(str scenario, list predclass_output, dict lf_dict):
    cdef list task_names
    cdef dict results
    cdef str task, task_module_path, sublabel, lf
    cdef list matching_lfs, data_points
    cdef object task_module, lf_list, lf_applier, sublabel_key
    cdef double lf_confidences
    cdef dict sublabels
    ...
    task_names = list(MODULE_MAPPING.keys())
...
def predict_scenario_logic(str request_scenario, request_version):
    """
    Predict scenario logic based on the provided scenario and version.
