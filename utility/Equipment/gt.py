...
def capability_multiple_fuel_acceptance(O):
    return 1 - O


def capability_vibration_suppression_util(K, L, O):
    if O >= K:
        return 1
    elif K > O > L:
        return 1 - ((K - O) / (K - L) * 0.1)
...
def gt_super_function(df, i):
    ...
    if i[1] == "Capacity (Efficiency) (%)":
    ...
    elif i[1] == "Capacity (Power) (MW)":
        # Process each row
        for index, row in df.iterrows():
            # Assuming C1 is K, C2 is L, and C5 is O in your original function
            result = capacity_power(row["C1"], row["C2"], row["C5"])
            results.append(result)
        results.append("Capacity (Power) (MW)")

    elif i[1] == "Capacity (Shaft Rotation) (RPM)":
        # Process each row
        for index, row in df.iterrows():
            # Assuming C1 is K, C2 is L, and C5 is O in your original function
            result = capacity(row["C1"], row["C2"], row["C5"])
    ...
    elif i[1] == "Capability (Multiple fuel acceptance) (%)":
        # Process each row
        for index, row in df.iterrows():
            # Assuming C1 is K, C2 is L, and C5 is O in your original function
            result = capability_multiple_fuel_acceptance(row["C5"])
            results.append(result)
        results.append("Capability (Multiple fuel acceptance) (%)")

    elif i[1] == "Capability (Vibration suppression)":
        # Process each row
        for index, row in df.iterrows():
            # Assuming C1 is K, C2 is L, and C5 is O in your original function
            result = capability_vibration_suppression_util(
        ...
        for index, row in df.iterrows():
            ...
            results.append(result)
        results.append(
            "Allowable Utilisation Factor (For equal utilisation of equipment)"
        )
