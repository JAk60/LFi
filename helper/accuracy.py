...
def accuracy_round_save_to_csv(output_text, labels, path):
    path = os.path.join(
        path, "accuracy_round.csv"
    )  # Adjusted to ensure proper path formatting
    ...
    for line in lines:
        ...
        for item in items:
            ...
            key_value = item.split(":")
            if (
                key_value[0].strip() != "best_epoch"
                and key_value[0].strip() != "score used"
            ):
                key = (
                    key_value[0].strip().replace("best_", "")
...
def cage_accuracy_save_to_csv(output_text, labels, path):
    ...
    with open(percentage_path, "a", newline="") as percentage_csv_file:
        ...
        if not percentage_csv_exists:
            percentage_csv_writer.writerow(
                [
                    "Task",
                    "Accuracy (%)",
                    "F1 Score (macro %)",
