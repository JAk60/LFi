...
class SentenceExtractor:
    ...
    def extract(self, sentence: str) -> Dict[str, Any]:
        ...
        prompt = """Extract time, location, action, and operational range/distance from this sentence. For naval contexts, distances are in nautical miles (nm). Return only these elements:""" + f"\n{sentence}"
            ...
            try:
                ...
                for line in parsed_response.split('\n'):
                    if 'time:' in line.lower():
                        ...
                        self.extracted_data['distance'] = self._extract_distance(line.split(':', 1)[1].strip())
                
                if 'distance' not in self.extracted_data:
                    distance_match = re.search(r'(\d+(?:\.\d+)?)\s*(?:nautical miles?|nm|nmi)', sentence, re.IGNORECASE)
                    if distance_match:
    ...
    def _extract_distance(self, distance_str: str) -> float:
        ...
        nm_match = re.search(r'(\d+(?:\.\d+)?)\s*(?:nautical miles?|nm|nmi)', distance_str, re.IGNORECASE)
        if nm_match:
